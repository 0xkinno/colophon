...
export function checkI1Determinism(run1, run2) {
...
export function checkI2SourceTraceability(intervals) {
    const untraced = intervals.filter((inv) => !inv.sourceSignature || inv.sourceSignature === "NONE" || inv.sourceSlot === 0n);
...
export function checkI3TemporalBoundary(effectiveAt, beforeActiveMultiplier, exactActiveMultiplier, afterActiveMultiplier, expectedOldMultiplier, expectedNewMultiplier) {
...
export function checkI4NoInventedHistory(confidence) {
...
export function checkI5RawBalanceConservation(rawBalance, transfersSum) {
...
export function checkI6TimelineMonotonicity(intervals) {
...
export function checkI7CurrentStateAgreement(reconstructedMultiplier, liveActiveMultiplier, tolerance = 1e-6) {
...
export function checkI8ProofDeterminism(stmt) {
...
export function checkI9KeySafety(bundleText) {
    const suspicious = [
        /private[_-]?key/i,
        /secret[_-]?key/i,
        /seed[_-]?phrase/i,
...
export function checkI10EvidenceLabelIntegrity(labels) {
    const invalid = labels.filter((l) => !VALID_EVIDENCE_LABELS.has(l));
